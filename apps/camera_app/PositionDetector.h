...
private:
    void extractMatches(std::vector<cv::DMatch> matches, float distance,
                                          std::vector<cv::KeyPoint> kp1, std::vector<cv::Point2f> &out_query, std::vector<int> &out_query_i,
                                          std::vector<cv::KeyPoint> kp2, std::vector<cv::Point2f> &out_train, std::vector<int> &out_train_i);

    double findR(cv::Mat _rot, cv::Mat _tr, const std::vector<int> &out_query_i, const std::vector<cv::Point2f> &out_train,
    ...
    cv::Mat m_prev_tr{};
    std::vector<cv::Point2f> m_prev_out_query;
    std::vector<cv::Point2f> m_prev_out_train;
